...
const addRole = async () => {
  ...
  const { title, salary, departmentId } = await inquirer.prompt([
    ...
    { type: 'input', name: 'salary', message: 'Enter salary:' },
    {
      ...
      choices: departments.rows.map((department: { department_name: string; department_id: number }) => 
        ({ 
...
const addEmployee = async () => {
  ...
  const { firstName, lastName, roleId, managerId } = await inquirer.prompt([
    ...
    { type: 'input', name: 'lastName', message: "Enter employee's last name:" },
    {
      ...
      choices: roles.rows.map((role: { role_title: string; role_id: number }) => 
        ({ 
    ...
    },
    {
      ...
      choices: [...employees.rows.map((employee: { first_name: string; last_name: string; employee_id: number }) => 
        ({ 
